    ...
    def map_value(self, source_value: int) -> int:
        ...
        for index, map_info in enumerate(self._maps):
            if source_value < map_info.source_range_start:
                ...
                break
            elif map_info.source_range_start <= source_value < map_info.source_range_start + map_info.range_length:
                destination_value = map_info.desintation_range_start + source_value - map_info.source_range_start
                range_to_end = map_info.range_length - (map_info.source_range_start - source_value)
                range_to_end /= 2
                break
            elif index + 1 < len(self._maps) and source_value < self._maps[index + 1].source_range_start:
                destination_value = source_value
...
if __name__ == '__main__':
    ...
    print(f"Lowest location value in seed range for puzzle 02: {puzzle_result.lowest_location_number_in_range()}")
